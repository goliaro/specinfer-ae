...
do
./FlexFlow/build/inference/spec_infer/spec_infer -ll:cpu 8 -ll:util 8 -ll:gpu 1 -ll:fsize 21000 -ll:zsize 80000 -llm-model $llm_model_name -ssm-model $ssm_model_name -prompt ./FlexFlow/inference/prompt/chatgpt_$bs.json --max-requests-per-batch $bs --expansion-degree -1 -offload -offload-reserve-space-size 500 --max-sequence-length 256 -output-file ./FlexFlow/inference/output/offloading_small_${bs}.txt > ./FlexFlow/inference/output/offloading_small_${bs}.out
done

llm_model_name="facebook/opt-30b"
for bs in "${batch_sizes[@]}"
do
./FlexFlow/build/inference/spec_infer/spec_infer -ll:cpu 8 -ll:util 8 -ll:gpu 1 -ll:fsize 21000 -ll:zsize 80000 -llm-model $llm_model_name -ssm-model $ssm_model_name -prompt ./FlexFlow/inference/prompt/chatgpt_$bs.json --max-requests-per-batch $bs --expansion-degree -1 -offload -offload-reserve-space-size 700 --max-sequence-length 256 -output-file ./FlexFlow/inference/output/offloading_large_${bs}.txt > ./FlexFlow/inference/output/offloading_large_${bs}.out
done
